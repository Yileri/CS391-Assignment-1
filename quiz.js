        ...
        if (qNumber >= 5) {
            ...
            for (let i = 0; i < quiz.length; i++) {
                ...
                if (quiz[i].studentAnswer == quiz[i].answer) {
                    let sumLineText = document.createTextNode("Question " + (i + 1) + "-----> " + quiz[i].question + "=" + 
                    quiz[i].studentAnswer + " and the answer is CORRECT!");
                    sumLine.appendChild(sumLineText);
                } else {
                    let sumLineText = document.createTextNode("Question " + (i + 1) + "-----> " + quiz[i].question + "=" + 
                    quiz[i].studentAnswer + " and the answer is WRONG! It should have been " + quiz[i].answer);
                    sumLine.appendChild(sumLineText);
                }
